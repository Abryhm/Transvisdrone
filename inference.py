...
def run(data,
    ...
    for batch_i, (img, paths, shapes, main_target_indices) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            image_tensor = img[si]  # [3, H, W]
            image = F.to_pil_image(image_tensor.cpu())
            image = np.array(image)[:, :, ::-1].copy()  # RGB to BGR for OpenCV

            pred = pred.cpu()
            if pred.numel() > 0:
                image = draw_boxes(image, pred)

            image_name = Path(paths[si]).stem + '_pred.jpg'
            cv2.imwrite(str(save_dir / image_name), image)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/NPS_original.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/T2/weights/last.pt', help='model.pt path(s)')
